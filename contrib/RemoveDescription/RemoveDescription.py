...
#
#-------------------------------------------------------------------------
class RemoveDescription(tool.BatchTool, ManagedWindow.ManagedWindow):
    """
    Look for events that have a generated description and remove it.
    ...
    def run(self):
            ...
            for person in self.db.iter_people():
                for event_ref in person.get_event_ref_list():
                    if event_ref.get_role() == gen.lib.EventRoleType.PRIMARY:
                        ...
                        description = person_event_name(event, person)
            ...
            for family in self.db.iter_families():
                for event_ref in family.get_event_ref_list():
                    if event_ref.get_role() == gen.lib.EventRoleType.FAMILY:
                        event_handle = event_ref.ref
                        event = self.db.get_event_from_handle(event_handle)
                        description = family_event_name(event, family, self.db)
                        if event.get_description() == description:
                            event.set_description("")
                            self.db.commit_event(event, trans)
                            self.change = True
        ...
        if self.change == True:
            OkDialog(_('Modifications made'), 
                    ngettext("%s event description has been removed", 
                    "%s event descriptions have been removed", counter)
                    % counter)
        else:
            OkDialog(_('No modifications made'), 
                    _("No event description has been removed."))
...
#
#------------------------------------------------------------------------
class RemoveDescriptionOptions(tool.ToolOptions):
    """
