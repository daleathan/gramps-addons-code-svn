...
import os
import gen.lib
from Utils import confidence, gender, media_path_full
...
class PersonEverythingReport(Report):
    ...
    def print_events(self, level, obj):
        for event_ref in obj.get_event_ref_list():
            ...
            text = ''
            if event.get_type() == gen.lib.EventType.BIRTH and \
                self.center_person.get_birth_ref() == event_ref:
                text = " This is the primary birth event."
            if event.get_type() == gen.lib.EventType.DEATH and \
    ...
    def print_object(self, level, o):
        ...
        if issubclass(o.__class__, gen.lib.address.Address):
        ...
        if isinstance(o, gen.lib.Citation):
            ...
            # printed here
            if o.get_confidence_level() != gen.lib.Citation.CONF_NORMAL:
                self.doc.start_paragraph("PE-Level%d" % min(level, 32))
                self.doc.start_bold()
                self.doc.write_text(_("Confidence") + " : ")
                self.doc.end_bold()
                self.doc.write_text(confidence[o.get_confidence_level()])
                self.doc.end_paragraph()
        ...
        if issubclass(o.__class__, gen.lib.datebase.DateBase):
            if o.get_date_object() and not o.get_date_object().is_empty():
                ...
                self.doc.end_bold()
                self.doc.write_text(DateHandler.displayer.display(
                                                        o.get_date_object()))
                self.doc.end_paragraph()
        ...
        if isinstance(o, gen.lib.LdsOrd):
            ...
            self.doc.end_bold()
            self.doc.write_text(", ".join((LdsUtils.TEMPLES.name(o.get_temple()),
                                           o.status2str()
                                   )))
            self.doc.end_paragraph()
        ...
        if isinstance(o, gen.lib.MediaRef):
            ...
            desc = gen.mime.get_description(mediaobject.get_mime_type())
            if not desc:
                desc = _("unknown")
        ...
        if isinstance(o, gen.lib.Note):
            ...
            self.doc.write_styled_note(o.get_styledtext(), 
                                       ...
                                       contains_html = o.get_type()
                                        == gen.lib.notetype.NoteType.HTML_CODE
                                      )
        ...
        if issubclass(o.__class__, gen.lib.Person):
            # This is printed by the main write-report function
            pass

        if isinstance(o, gen.lib.Place):
            # The title and lat/long are printed by PlaceBase
        ...
        if isinstance(o, gen.lib.Surname):
            if o.get_origintype():
        ...
        if isinstance(o, gen.lib.surnamebase.SurnameBase):
        ...
        if issubclass(o.__class__, gen.lib.Tag):
...
def _citation_has_details(citation):
    return citation.get_media_list() or \
           citation.get_data_map() or \
           citation.get_confidence_level() != gen.lib.Citation.CONF_NORMAL or \
