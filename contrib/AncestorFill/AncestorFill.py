...
from gramps.gen.plug.docgen import (IndexMark, FontStyle, ParagraphStyle, FONT_SANS_SERIF, INDEX_TYPE_TOC, PARA_ALIGN_CENTER)
...
class AncestorFillReport(Report):
    ...
    def write_report(self):
        ...
        gettext = self._locale.translation.gettext
        ...
        for gen in range(0, self.max_generations):
            nextgen = gen + 1
            for gid in list(self.gener[gen].keys()):
            ...
            nbhand = 0
            msg = "GEN  " + str(nextgen)
            for hand in list(self.gener[nextgen].keys()):
                msg = msg + " " + str(hand)
        ...
        text = gettext(form %(total)) + " " + _(form2 %(totalnbanc)) + _(form3 % (timplexe))
        self.doc.start_paragraph("AHN-Generation")
        self.doc.write_text(text)
        self.doc.end_paragraph()
...
class AncestorFillOptions(MenuReportOptions):
    ...
    def add_menu_options(self, menu):
        ...
        menu.add_option(category, "maxgen", maxgen)
        ...
        menu.add_option(category, "Filleddigit", Filleddigit)
        ...
        menu.add_option(category, "Collapsedigit", Collapsedigit)
        ...
        menu.add_option(category, "Display theorical", displayth)
