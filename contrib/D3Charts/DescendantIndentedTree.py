...
from gen.proxy import LivingProxyDb, PrivateProxyDb
...
class Printinfo():
    ...
    def __init__(self, database, numbering, showmarriage, showdivorce,\
                 ...
                 compute_age, verbose, inc_photo, rep_place, rep_date,
                 trans, hrefs, href_prefix, href_ext, href_delim, href_gen,
        ...
        self.translator = Translator(trans)
            ...
            if len(media_list) > 0:
                ...
                if mime_type and mime_type.startswith("image"):
                    ...
                    if os.path.exists(filename):
                        ...
                        ImgManip.resize_to_jpeg(filename, image_file,
                                                size, size, None)
    ...
    def generate_href_link(self, name):
        ...
        if name_out in self.href_dict:
            ...
            name_out = name_out + delim + str(self.href_dict[name_out])
...
class DescendantIndentedTreeReport(Report):
    ...
    def __init__(self, database, options, user):
        ...
        self.destexpandedjs = unicode(os.path.join(
        ...
        self.objPrint = Printinfo(self.database, self.num_obj, self.marrs,
                                  ...
                                  self.use_fulldates,
                                  self.compute_age, self.verbose,
                                  self.inc_photo, self.rep_place,
                                  self.rep_date, self.trans, self.hrefs,
                                  self.href_prefix, self.href_ext,
    ...
    def write_report(self):
        ...
        if not os.path.isdir(self.dest_path):
            prompt = OptionDialog(_('Invalid Destination Directory'),
                                  _('Destinaton diretory %s does not exist\n'
                                    'Do you want to attempt to create it.') %
                                  self.dest_path,
                                  _('_No'), None,
                                  _('_Yes'), None)
            if prompt.get_response() == RESPONSE_YES:
                try:
        ...
        if os.path.isfile(self.desthtml):
            prompt = OptionDialog(_('File already exists'),
        ...
        # Generate json data file to be used
        try:
...
class DescendantIndentedTreeOptions(MenuReportOptions):
    ...
    def add_menu_options(self, menu):
        ...
        self.contraction = NumberOption(_("Initial contraction level"),
                                        3, MIN_GEN, MAX_GEN)
        ...
        # Contents
        add_option = partial(menu.add_option, _("Contents"))
